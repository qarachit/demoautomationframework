...
public class TestNGCustomReportListener implements IReporter{
	...
	public String browserType = propertyReader.readApplicationFile("BROWSER","//PlanningAndConsolidation//com//adaptiveinsight//config//application.properties");
	...
	protected void generateMethodDetailReport(List<ISuite> suites) {
		...
		for (ISuite suite : suites) {
			...
			for (ISuiteResult r2 : r.values()) {
				...
				resultDetail(testContext.getFailedConfigurations(),"#FF0000");
				resultDetail(testContext.getFailedTests(),"#FF0000");
				resultDetail(testContext.getSkippedConfigurations(),"#8F938F");
				resultDetail(testContext.getSkippedTests(),"#8F938F");
				resultDetail(testContext.getPassedTests(),"#33FF46");
	...
	private void resultDetail(IResultMap tests, String color) {
		...
		for (ITestResult result : testResultsList) {
			...
			writer.println("<h2 bgcolor=\""+color + "\", id=m" + m_methodIndex + ">" + cname + ":"
	...
	public void generateSuiteSummaryReport(List<ISuite> suites) {
		...
		for (ISuite suite : suites) {
			...
			for (ISuiteResult r : tests.values()) {
				...
				qty_pass_m += q;
				summaryCell(q, Integer.MAX_VALUE,"#26c935");
				q = getMethodSet(overview.getSkippedTests(), suite).size();
				qty_skip += q;
				if (q > 0)
				{summaryCell(q, 0,"#8F938F");}
				else {summaryCell(q, 0,"");}
				q = getMethodSet(overview.getFailedTests(), suite).size();
				qty_fail += q;
				if (q > 0)
				{summaryCell(q, 0,"#FF0000");}
				else {summaryCell(q, 0,"");}
				
				int total = getMethodSet(overview.getPassedTests(), suite).size() + getMethodSet(overview.getSkippedTests(), suite).size() + getMethodSet(overview.getFailedTests(), suite).size();
		...
		if (qty_tests > 1) {
			...
			summaryCell(qty_pass_m, Integer.MAX_VALUE,"#26C935");
			if (qty_skip > 0)
			{summaryCell(qty_skip, 0,"#8F938F");}
			else
			{summaryCell(qty_skip, 0,"");}
			if (qty_fail > 0)
			{summaryCell(qty_fail, 0,"#FF0000");}
			else
			{summaryCell(qty_fail, 0,"");}
			summaryCell(qty_pass_m+qty_skip+qty_fail, 0,"");
			summaryCell(" ", true,"");
			summaryCell(" ", true,"");
	...
	private void summaryCell(String v, boolean isgood, String color) {
		writer.print("<td bgcolor=\""+color + "\", class=numi" + (isgood ? "" : "") + ">" + v
				+ "</td>");
	}
	...
	private void summaryCell(int v, int maxexpected, String color) {
		summaryCell(String.valueOf(v), v <= maxexpected, color);
	}
	...
	protected void writeReportTitle(String title, List<ISuite> suites) {
		...
		writer.print("<center><h2>" + suiteName + " " + title + " - on " + browserType  + " - " + getDateAsString() + "</h2></center>");
	}
	...
	protected void startHtml(PrintWriter out) {
		...
		out.println(".passedodd td {background-color: #26C935}");
		out.println(".passedeven td {background-color: #26C935}");
		out.println(".skippedodd td {background-color: #8F938F}");
		out.println(".skippedeven td {background-color: #8F938F}");
